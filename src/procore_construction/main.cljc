(ns procore_construction.main
  "Kotodama WASM entrypoint for the procore_construction clean-room actor (L4) — Clojure port.
  ...
  facts (`procore_construction.<Entity>/<field>`); `*store*` is the in-memory
  materialization used by the contract test and by the WASM runtime before a
  live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "procore_construction")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"procore_construction.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "procore_construction-compat" :tier tier :entities entities} 200])
...
(defn start! [] :procore_construction-compat/ready)
