(ns procore_construction.main-test
  "Contract + behavioral test for the procore_construction-compat L4 actor (cljc port).
  ...
  (:require [clojure.test :refer [deftest is testing run-tests]]
            ...
            [procore_construction.main :as m]))
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "procore_construction-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'procore_construction.main-test)]
